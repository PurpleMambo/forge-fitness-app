...
private let steps: [OStep] = [
    ...
    ], options: ["Lift heavier", "Build more muscle", "Get lean and defined", "Lose weight"]),

    OStep(parts: [
        ("Great, and which best describes your current strength training ", false),
        ("routine?", true)
    ], options: ["I'm just getting started", "I struggle with consistency", "I'm coming back after a break", "I strength train consistently"]),

    OStep(parts: [
        ("Got it, we'll tailor your plan to your experience.\n\nNext, ", false),
        ("where", true),
        (" do you primarily plan on training?", false)
    ], options: ["At a large commercial gym", "At a small gym", "In a garage gym", "At home with limited equipment", "I don't have any equipment"]),

    OStep(parts: [
        ("Nice. How many ", false),
        ("days per week", true),
        (" can you usually strength train?", false)
    ], options: ["1-2 days a week", "3 days a week", "4 days a week", "5 days a week", "6+ days a week"]),
    ...
    ], options: ["Yes, enable notifications", "Maybe later"]),

    OStep(parts: [
        ("No worries, you can enable that later.\n\nLastly, do you want to calculate the ", false),
        ("calories you burn", true),
        (" each workout?", false)
    ], options: ["Yes, Connect to Apple Health", "Yes, input details manually", "Maybe later"]),
]
...
struct OnboardingView: View {
    ...
    var chatBody: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            // Previous answer bubble — uses Liquid Glass (clear, since it's over the dark background)
            ...
            // Answer option buttons — Liquid Glass pill buttons in a container for morph effect
            if showOptions, step < steps.count {
                GlassEffectContainer(spacing: 10) {
                    VStack(alignment: .trailing, spacing: 10) {
                        ForEach(Array(steps[step].options.enumerated()), id: \.offset) { _, opt in
                            HStack {
                                Spacer()
                                Button(opt) { pick(opt) }
                                    .buttonStyle(.glass)
